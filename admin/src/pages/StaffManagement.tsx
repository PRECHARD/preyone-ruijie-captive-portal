...
export default function StaffManagement() {
  ...
  const handleAction = async (action: string, id: string) => {
    ...
    try {
      ...
      setMsg(`Staff ${action}d successfully`);
      fetch();
  ...
  return (
    <div className="page">
      ...
      {msg && (
        <div className={`form-status ${msg.includes('successfully') || msg.includes('approved') || msg.includes('activated') ? 'form-status--success' : msg.includes('rejected') || msg.includes('deactivated') ? 'form-status--cyan' : 'form-status--error'}`} style={{ marginBottom: 16 }}>
          {msg}
        </div>
      )}
